...
class MDD:
    ...
    def __init__(
        self, my_map, start_loc, h_values, num_levels, positive_table, negative_table
    ):
        ...
        while len(self.open_list) > 0:
            ...
            for dir in range(5):
                ...
                if (
                    child_loc[0] < 0
                    or child_loc[1] < 0
                    or child_loc[0] >= len(self.my_map)
                    or child_loc[1] >= len(self.my_map[0])
                ):
                    continue
                ...
                if is_constrained(
                    curr.location, child_loc, curr.level + 1, self.negative_table
                ):
                    continue
                ...
                if (
                    is_positive_satisfied(
                        parent_node=p_node,
                        child_node=c_node,
                        next_time=curr.level + 1,
                        positive_constraints=self.positive_table,
                    )
                    == False
                ):
                    continue
