...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints, isMDD=False):
    ...
    positive_table = []
    if constraints != None:
        [negative_table, positive_table] = build_constraint_tables(
            constraints=constraints, agent=agent
        )
    ...
    while len(open_list) > 0:
        ...
        for dir in range(5):
            ...
            if is_constrained(
                curr["loc"], child_loc, curr["time_step"] + 1, negative_table
            ):
                continue
            ...
            if (
                is_positive_satisfied(
                    ...
                    positive_constraints=positive_table,
                )
                == False
            ):
                continue
