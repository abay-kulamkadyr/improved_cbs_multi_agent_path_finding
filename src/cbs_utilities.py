...
def detect_collision(path1, path2):
    ...
    for time_step in range(max_time_steps):
        ...
        if agent1_loc_curr == agent2_loc_prev and agent1_loc_prev == agent2_loc_curr:
            return {
                "loc": [agent2_loc_curr, agent2_loc_prev],
...
def standard_splitting(collision):
    ...
    if len(collision_loc) == 1:
        return [
            {"agent": agent1_id, "loc": collision_loc, "time_step": time_step},
...
def disjoint_splitting(collision):
    ...
    # if edge collision
        ...
        if chosen_agent == 0:
            ...
            loc = collision_loc
        return [
            {"positive": True, "agent": agent, "loc": loc, "time_step": time_step},
...
def getBetterCollision(collisions, mdds):
    for collision in collisions:
        ...
        time_step = collision["time_step"]
        if (
            len(agent1_mdd.levels) - 1 < time_step
            or len(agent2_mdd.levels) - 1 < time_step
        ):
            continue
