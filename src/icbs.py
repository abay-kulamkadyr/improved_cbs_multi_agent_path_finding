...
class ICBSSolver(object):
    ...
    def computeInformedHeuristics(self, collisions, curr, parent, heuristicType):
        ...
        if heuristicType == 0:
            ...
            CG, num_of_CGedges = self.buildCardinalConflictGraph(
                collisions, HG, curr["mdds"]
            )

            if parent is None:
                h = minimumVertexCoverHelper(CG, self.num_of_agents)
            else:
                h = minimumVertexCover(
                    CG, parent["h_value"], self.num_of_agents, num_of_CGedges
                )
            ...
            if parent is None:
                ...
                h = minimumVertexCover(
                    DG, parent["h_value"], self.num_of_agents, num_of_DGedges
                )
        elif heuristicType == 3:  # WDG
            WDG, isFound = self.buildEdgeWeightedDependancyGraph(
                collisions, HG, curr["mdds"], curr["constraints"]
            )
        ...
        for collision in collisions:
            ...
            if areAgentsDependent(mdd1, mdd2):
                ...
                if DG[a1 * self.num_of_agents + a2] == 0:
                    ...
                    conflict_free_cost = self.solve2Agents(
                        self.my_map,
                        [self.starts[a1], self.starts[a2]],
                        [self.goals[a1], self.goals[a2]],
                    )

                    if conflict_free_cost == None:
                        continue

                    value = conflict_free_cost - (
                        get_sum_of_cost(optimal_path_a1)
                        - 1
                        + get_sum_of_cost(optimal_path_a2)
                        - 1
                    )
    ...
    def find_solution(self, disjoint=False, h=0, initialConstraints=[]):
        ...
        success, root["h_value"] = self.computeInformedHeuristics(
            root["collisions"], root, None, h
        )
        ...
        while len(self.open_list) > 0:
            ...
            if len(parent_node["collisions"]) == 0:  # if no collisions return paths
                ...
                if h != 0:
                    return (
                        ...
                        self.num_of_generated,
                        parent_node["paths"],
                        (self.h_values_cumulitive * 1.0) / self.h_values_total,
                    )
                else:
                    return (
                        CPU_time,
            ...
            collision = getBetterCollision(
                parent_node["collisions"], parent_node["mdds"]
            )
                ...
                if path is not None:
                    if constraint["positive"]:
                        ...
                        for agent in broken_agents_paths:
                            ...
                            updated_path, updated_mdd = a_star(
                                my_map=self.my_map,
                    ...
                    success, Q["h_value"] = self.computeInformedHeuristics(
                        Q["collisions"], Q, parent_node, h
                    )
                    if not success:
                        print("h<0")
