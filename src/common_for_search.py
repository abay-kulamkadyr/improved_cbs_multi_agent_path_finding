    ...
    for constraint in constraints:
        ...
        if constraint["agent"] == agent:
            ...
            if constraint["positive"] == False:
                ...
                if (
                    constraint["time_step"] in negative_constraint_table
                ):  # if table constains constraints in the time_step, append to it
                    negative_constraint_table[constraint["time_step"]].append(
                        constraint["loc"]
                    )
                else:
                    negative_constraint_table[constraint["time_step"]] = [
                        constraint["loc"]
                    ]  # if not, create a new time_step/location key/value pair

            if constraint["positive"] == True:

                if (
                    constraint["time_step"] in positive_constraint_table
                ):  # if table constains constraints in the time_step, append to it
                    positive_constraint_table[constraint["time_step"]].append(
                        constraint["loc"]
                    )
                else:
                    positive_constraint_table[constraint["time_step"]] = [
                        constraint["loc"]
                    ]  # if not, create a new time_step/location key/value pair
        else:
            if constraint["positive"] == True:
                ...
                if (
                    new_constraint["time_step"] in negative_constraint_table
                ):  # if table constains constraints in the time_step, append to it
                    negative_constraint_table[new_constraint["time_step"]].append(
                        new_constraint["loc"]
                    )
                else:
                    negative_constraint_table[new_constraint["time_step"]] = [
                        new_constraint["loc"]
                    ]
...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    if next_time in constraint_table:  # if next_time key exists in the constraint_table
        constrained_cells = constraint_table[
            next_time
        ]  # obtained constrained cells/edges at the given time step
    ...
    if next_time in positive_constraints:
        ...
        for constraint in constrained_cells:
            ...
            if len(constraint) == 1:
                ...
                if (parent_node["loc"] != constraint[0]) or (
                    child_node["loc"] != constraint[1]
                ):
                    return False
    return True
